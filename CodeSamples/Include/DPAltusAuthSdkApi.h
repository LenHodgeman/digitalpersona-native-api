...
DigitalPersona Altus Authentication SDK is a library that allows you to authenticate Altus users in your applications.  

The SDK leverages the DigitalPersona Altus multi-factor authentication capability:  authentication is performed using 
the Altus admin's authentication policy and all credential types supported by Altus (passwords, fingerprints, smart cards, 
etc.) are supported by the API.


OVERVIEW

DigitalPersona Altus Authentication SDK provides C++ functions to authenticate Altus users and release
...
provided credentials (password, fingerprint, etc.) match their username's credentials in the Altus database.
...
2. Identification:  Searching in Altus database to find the user and authenticate them.
...
authentication of the user. Secrets are stored in the Altus database and roam with the rest of the user data. 


The most common use case is to add standard Altus authentication or identification to an application. 

For advanced users, the API provides for custom authentication policies that allow you to:
   - Extend the Altus admin's policy for more strict authentication (e.g. to add a credentials requirement)
        OR 
   - Create your own authentication policy. In this case, it may not be possible to retrieve secrets if your 
     new policy is not as strict as the Altus admin's policy
...
typedef enum DPAl_USERNAME_TYPES {
	...
	DPAl_USER_NAME_USER_SID       = 8, // A user SID, for example, L"S-1-5-21-1496820208-3081509908-3797829617-1001"
...
typedef HRESULT(STDAPICALLTYPE *pfnDPAlInit)();
...
typedef HRESULT(STDAPICALLTYPE *pfnDPAlTerm)();
...
// to that user's enrolled credentials stored in the Altus database.
//
// Returned Values:
//
...
//
//   HR(ERROR_BAD_ENVIRONMENT)      This function was called from a DigitalPersona Altus Kiosk session with the name of the generic Kiosk user. 
//
//   Other COM errors.
//
DPAl_STDAPI DPAlAuthenticate(
	...
	LPCWSTR szUserName,                // [in, optional] Pointer to NULL-terminated string with username to authenticate. 
	                                   ...
	                                   // or name associated with DP Altus identity database (any string). If set to NULL, the currently logged on user is used. 
...
typedef HRESULT(STDAPICALLTYPE *pfnDPAlAuthenticate)(HWND hParentWnd, LPCWSTR szCaption, LPCWSTR szText, LPCWSTR szUserName, ULONG uUserNameType, UINT64* pPolicyArray, ULONG nNumPolicyArrayItems, LPCWSTR szSecretName, ULONG* puSecretLength, LPBYTE* ppSecretBuffer, UINT64* puAuthenticatedCredentials);
...
typedef HRESULT(STDAPICALLTYPE *pfnDPAlIdentAuthenticate)(HWND hParentWnd, LPCWSTR szCaption, LPCWSTR szText, LPCWSTR szDefaultUserName, ULONG uDefaultUserNameType, UINT64* pPolicyArray, ULONG nNumPolicyArrayItems, LPCWSTR szSecretName, ULONG* pdwSecretLength, LPBYTE* ppSecretBuffer, LPWSTR* pszUserName, ULONG* puUserNameType, UINT64* puAuthenticatedCredentials);
...
typedef HRESULT(STDAPICALLTYPE *pfnDPAlReadAuthPolicy)(LPCWSTR szUserName, ULONG uUserNameType, UINT64** pPolicyArray, ULONG* pnNumPolicyArrayItems);
...
typedef HRESULT(STDAPICALLTYPE *pfnDPAlWriteSecret)(HWND hParentWnd, LPCWSTR szCaption, LPCWSTR szText, LPCWSTR szUserName, ULONG uUserNameType, UINT64* pPolicyArray, ULONG nNumPolicyArrayItems, LPCWSTR szSecretName, ULONG dwSecretLength, const LPBYTE pSecretBuffer, UINT64* puAuthenticatedCredentials);
...
typedef HRESULT(STDAPICALLTYPE *pfnDPAlDoesSecretExist)(LPCWSTR szUserName, ULONG uUserNameType, LPCWSTR szSecretName);
...
typedef HRESULT(STDAPICALLTYPE *pfnDPAlBufferFree)(PVOID pBuffer);
...
typedef HRESULT(STDAPICALLTYPE *pfnDPAlFormatMessage)(HRESULT hrMessageID, LPCWSTR szLanguageID, LPWSTR* pszMessageText);
...
typedef HRESULT(STDAPICALLTYPE *pfnDPAlVerifyUserUI)(HWND hParentWnd, LPCWSTR szCaption, LPCWSTR szText, ULONG hOperation, UINT64* pPolicyArray, ULONG nNumPolicyArrayItems);
...
typedef HRESULT(STDAPICALLTYPE *pfnDPAlIdentifyUserUI)(HWND hParentWnd, LPCWSTR szCaption, LPCWSTR szText, LPCWSTR szDefaultUserName, ULONG uDefaultUserNameType, UINT64* pPolicyArray, ULONG nNumPolicyArrayItems, LPWSTR* pszUserName, ULONG* uUserNameType, ULONG* phOperation);

//////////////////
DPAl_STDAPI DPAlIdentifyUserUI1(HWND hParentWnd, LPCWSTR szCaption, LPCWSTR szText, LPCWSTR szDefaultUserName, ULONG uDefaultUserNameType, ULONG uUserNameTypeMask, UINT64* pPolicyArray, ULONG nNumPolicyArrayItems, LPWSTR* pszUserName, ULONG* uUserNameType, ULONG* phOperation);
// Most of the Paramaters are intuitive enough or described in the published API functions above.
// New parameters:
//     uUserNameTypeMask: Users to allow for identification (show user type selector in UI). 
//                        This is mask of the user types: 
//                                                    0x1 is AD (Windows) users 
//                                                    0x2 - Altus users 
//                                                    New user types may be added in the future. 
//                                                    0x0 - all supported user types.
//                        Currently supported values: 0x0 or 0x3: AD + Altus users; 0x1: AD/Windows users; 0x2 - Altus users.
//
//     phOperation:       Operation handle. Low-level interface: you must know what are you doing if using it.
typedef HRESULT(STDAPICALLTYPE *pfnDPAlIdentifyUserUI1)(HWND hParentWnd, LPCWSTR szCaption, LPCWSTR szText, LPCWSTR szDefaultUserName, ULONG uDefaultUserNameType, ULONG uUserNameTypeMask, UINT64* pPolicyArray, ULONG nNumPolicyArrayItems, LPWSTR* pszUserName, ULONG* uUserNameType, ULONG* phOperation);
...
typedef HRESULT(STDAPICALLTYPE *pfnDPAlWriteSecretUI)(HWND hParentWnd, LPCWSTR szCaption, LPCWSTR szText, LPCWSTR szUserName, ULONG uUserNameType, UINT64* pPolicyArray, ULONG nNumPolicyArrayItems, LPCWSTR szSecretName, DWORD dwSecretLength, LPBYTE pSecretBuffer, ULONG64* puAuthneticatedCredentials);
